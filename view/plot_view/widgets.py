...
class ConfigureTransformationPopup(QDialog):
    def __init__(self, indices, particle_names, parent=None):
        ...
        # V_Y_hbox.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        ...
        V_Y_hbox.addWidget(self.vector_pair_label) #, alignment=Qt.AlignLeft)
        V_Y_hbox.addWidget(swap_button) #), alignment=Qt.AlignLeft)
        V_Y_hbox.addStretch()
        layout.addLayout(V_Y_hbox)

        hbox_rest_frame = QHBoxLayout()
        hbox_rest_frame.setSpacing(0)
        rest_frame_label = QLabel("Configure with")
        rest_frame_label_font = QLabel().font()
        rest_frame_label_font.setPointSize(12)
        rest_frame_label.setFont(rest_frame_label_font)
        self.rest_frame_checkboxes_group = QButtonGroup()
        self.rest_frame_checkboxes_group.setExclusive(True)
        self.V_rest_frame_checkbox = QCheckBox("V")
